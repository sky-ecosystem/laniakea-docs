...
The P1 structural-demand allocator is the canonical case. Synserv triggers it during the daily processing window. It reads bucket capacities, Sky Prime token-share facts, and per-Prime IJRC, then writes ownership-weighted pro-rata allocation atoms. Phase 9+ replaces the body with real OSRC + Duration auction matching. The trigger, read path, atom shape, and structbook consumption site stay fixed.

**Temporary-equation containment:** all temporary logic must live inside the body that will later be swapped. Long-term systems it consumes stay clean. For P1, `&core.treasury` stores raw Sky token-share facts; the ownership-weight calculation lives entirely in `&entity.generator.usge.auction`.
...
The same shape recurs for **loop bodies** (per-entity loop Spaces hold their bodies in P1; canonical templates ship later, propagating into the same per-entity Spaces — entarts can also keep local extensions alongside the propagated body) and for **structural-demand allocations** (the auction Space holds synserv-written pro-rata SDR allocations in P1; the real Prime-strategy-driven auction later writes the same atom shape into the same Space).
...
Later settlement closure, TMF, DR/LDR, and real auctions add more processing tasks to the same cadence.
...
The conversation iterated through several scopes; v4 (2026-05-17 live-state cleanup, building on the 2026-05-15 topology redesign) settled on:
...
3. **Risk framework via per-halo risk class** — one risk class (`custodial-crypto`) per halo, each carrying the stress-envelope risk form (real synlang over market-memory reducer outputs, chain reads, and attestation gates); no canonical `&core.framework.*` Space in P1.
...
What's in Phase 1 (v4):
- **66 fixed Spaces** (6 universal Core + 4 Generator + 3 Oracle + 7 Primes × 5 + 3 Halos × 6)
...
2. **`phase-2-spaces.md`** — Phase 2 adds formal settlement closure tasks to the existing daily synomic settlement cycle; what Spaces/atoms does closure introduce (in-space settlement verification, prepayment / penalty event records)? Plus the test additions for Phase 2.
