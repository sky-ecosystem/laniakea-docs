...
| **TTS** | Time to Shutdown — worst-case time for wardens to detect and halt a rogue sentinel; determines ORC requirements. In Phase 1 (pre-sentinel) the same term covers the detection-to-halt window (24h) for compromised GovOps guardians. See `risk-framework/operational-risk-capital.md` |
