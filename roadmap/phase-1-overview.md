...
**Spaces it covers.** `&core.syngate` (trust boundary), `&core.loop.synserv` (heartbeat loop body), `&core.registry.beacon` (beacon identities + cert/auth — absorbs the legacy Guardian root), `&core.settlement` (DSC state), `&core.treasury` (Sky token-share facts), the grounded-atom surface (no registry Space — it's runtime). `&core.test-suite` is core substrate but counted under test-runner.
...
**Spaces it covers.** `&core.loop.synserv` rollup body; the 7 prime entarts (`root` / `primebook` / `structbook` / `relay` / `protocol-registry`); the 3 halo entarts (`root` / `nfat-term` halo class / `custodial-crypto` risk class / `custodial-crypto.attest-data` attestor loop / `relay` / `protocol-registry`); constructor-made `halobook` / `riskbook` / `exobook`; market-memory atoms from `&entity.oracle.crypto-majors.ticks`; attestor gates; **patch beacon + insyn/exsyn TRRC split** (`TRRC = insynTRRC + exsynTRRC`, exsyn from `patch-{prime}` into each primebook); govops verbs; the NFAT instrument; rollup path `exobook → riskbook → halobook → NFAT → primebook → ER`.
...
**What it does.** Runs the canonical heartbeat (`&core.loop.synserv`), is the sole sequencer of synart writes, evaluates the rollup, emits `(prime-er …)` per tick. Singleton. **Guardian (Ozone)** is the rooting authority — in P1 it holds all sudo authority and the cert chain; genesis is a sequence of Guardian sudo writes.
...
**What it does.** Human-in-the-loop operational layer — P1 beacons are deterministic teleonome-less programs; govops is humans deciding what txs to send. **Prime-side** (`govops-prime`): deploy / rollover / withdraw. **Halo-side** (`govops-halo`): runs the constructors (`create-halobook` / `-riskbook` / `-exobook`), `record-unit`, lifecycle transitions, `update-exobook-state`.

**Challenges + watch out.** Where the dynamic auto-wiring challenge bites — govops actions must produce immediately-live books with no follow-up sudo. Also certs the patch beacon.

### Attestor Oracle (Book Attestation Oracle)

**What it does.** Rank-2 Oracle Entity owning the attestor cert chain; operates `attest-data` beacons writing boolean borrower-admission, riskbook, and exobook attestation atoms. For custodial-crypto it is *not* an oracle of loan facts (those are insyn) — it underwrites legal structure, borrower credit standing, account binding, custodian compliance, and exobook term enforceability. Full schema: [`attestor-atom-schema.md`](attestor-atom-schema.md).

**Operational shape.** Underwrite new loan setup → sign boolean. `refresh-due` approaches → re-underwrite (credit-review cadence, not market cadence). Material structural change (`scope-ref` mismatch) → re-attest. Stale → exobook drops from rollup (default-deny).
