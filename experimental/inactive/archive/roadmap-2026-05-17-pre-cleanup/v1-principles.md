...
1. **Liability duration determines duration capacity.** The Lindy Duration Model is the target measurement; P1 uses governance-set bucket capacities at the same consumption site until the lot-age endoscraper surface exists.
...
3. **Duration matching protects against credit-spread and liquidity risk by making the position hold-to-par.** Credit spreads are mean-reverting; forced-sale liquidity loss is avoided when the Prime can hold.
...
15. **Temporary equations must be contained.** The P1 ownership-weighted pro-rata SDR allocator lives entirely inside `&entity.generator.usge.auction`; `&core.treasury` stores only fundamental token-share facts so the temporary body can be swapped out cleanly. Structbooks consume only the allocation atom shape, so later auctions can replace the writer without changing the book read path.
