# Distribution Rewards, Liability Duration Rewards, Pioneer Star System

**Status:** Live (DR + LDR active); Pioneer Star System available for the 5 Star Primes
...
| **Liability Duration Rewards (LDR)** | Sourcing sticky USDS demand that supports duration-matched asset deployment | 2/3 of duration-bucket fees to the tagging Prime, 1/3 to Sky |
...
DR and LDR are paid out at each settlement cycle. The Pioneer Star System is a separate set of privileges available only to **Star Primes** (the five genesis-named Primes: Spark, Grove, Keel, skybase, launch6). The Institutional Prime (Obex) does not participate in the Pioneer System.
...
| **1** | 0 bps | Untagged addresses (tracked but unpaid; can still earn LDR) |
...
| Property | Value |
|---|---|
| Tag duration | 10 years from tagging |
...
LDR rewards Primes that source **tagged USDS demand feeding into the Asset-Liability Duration Matching (ALDM) system**. Where DR rewards adoption (any USDS held by an end user), LDR rewards adoption that produces *sticky* duration that matches against long-duration assets.

| Property | Value |
|---|---|
| Purpose | Compensate Primes for sourcing duration capacity (sticky USDS demand) |
...
1. USDS accounts accrue **liability duration** over time and land in [Duration Buckets](../risk-framework/duration-model.md) based on their measured holding duration.
2. Primes that have tagged those USDS accounts earn a proportional share of the bucket's underlying demand.
3. When a Prime pays fees to reserve duration capacity, their reservation **tugs** on buckets to match their need (the tug-of-war mechanism in [`duration-allocation.md`](duration-allocation.md)).
...
Even **Tier 1 (0 bps DR)** tagged addresses can earn LDR. This lets Primes benefit from demand-side contributions without requiring full branding or composition requirements — a generic USDS holder that happens to be sticky still produces value when matched against long-duration assets.
...
"Unrewarded USDS" = bridged USDS that is **not** earning SSR through sUSDS, Integration mechanisms, or holdings in other Synomic Entities. The Pioneer Star earns SSR-equivalent on this otherwise-idle balance for the duration of the Pioneer Phase.
...
| [`entity-fees.md`](entity-fees.md) | Sky → Entity revenue (opposite direction from DR/LDR/Pioneer Rewards) |
| [`treasury-management.md`](treasury-management.md) | DR / LDR / Pioneer Rewards are paid from Sky's gross revenue before it enters the TMF waterfall |
| [`duration-allocation.md`](duration-allocation.md) | Duration Bucket reservation and tug-of-war that LDR distributes fees from |
| [`../risk-framework/duration-model.md`](../risk-framework/duration-model.md) | Lindy Duration Model and Duration Buckets that house tagged USDS |
...
| [`../synomic-entities/generator.md`](../synomic-entities/generator.md) | USDS Generator that issues the assets DR/LDR rewards adoption of |
